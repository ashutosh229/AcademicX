...
class Student(models.Model):
    ...
    )  # Allow blank for manual override
...
class CourseMetrics(models.Model):
    ...
    )  # by default pk, to_field not needed
    """
...
class Comment(models.Model):
    ...
    contributor = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="comments", to_field="email"
    )
    is_anonymous = models.BooleanField(default=False)
    upvotes = models.PositiveIntegerField(default=0)
    downvotes = models.PositiveIntegerField(default=0)
...
class ResourceVote(models.Model):
    ...
    resource = models.ForeignKey(
        Resource, on_delete=models.CASCADE, related_name="votes"
    )
...
class CommentVote(models.Model):
    ...
    vote_type = models.IntegerField(
        choices=[(2, "Downvote"), (1, "Upvote")]
    )  # restricts choices to valid values

    class Meta:
        unique_together = (
            "student",
            "comment",
        )  # Ensures one vote per student per comment
