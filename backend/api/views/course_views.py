...
def give_course_feedback(request):
    ...
    if not course_id or not contributor_email:
        return Response({"error": "Course ID and contributor email are required."}, status=status.HTTP_400_BAD_REQUEST)

    # Check if the contributor has already given feedback for this course
    if CourseMetrics.objects.filter(course_id=course_id, contributor_id=contributor_email).exists():
        return Response({"error": "You have already given feedback for this course."},
